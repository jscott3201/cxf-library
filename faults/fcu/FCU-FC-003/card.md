...
This is the cooling-side member of the reference's FCU-FC-002/003 pair, the fan
coil analog of AHU-FC-007/013.
...
present at load. The graph is FCU-FC-002's mirror — same five blocks, same two
...
Two readings that list needs. The reference gives FCU-FC-002 "SAT sensor out of
...
for twice (FCU-FC-005 sees that), while a fouled coil or warm chilled water
...
- **Both tunables are adopted, not transcribed.** The reference's card states an
  ...
  provenance NISTIR 7365), the values AHU-FC-013 ships. The delay is the more
  debatable — a fan coil settles far faster than an air handler, so a site can
  drop it to 900 s — and the chapter's one published AlarmDelay (FCU-FC-001's
...
- **The setpoint comparison is rewritten as a gap comparison**, the same
  rearrangement as AHU-FC-013 and AHU-FC-001: subtracting first keeps the
...
- **Instantaneous samples, with no averaging.** The reference writes the equation
  ...
  does too; if it does, this rule differs the way AHU-FC-013 documents. Averaging
...
- **`yClgFullOk` is the library's, not the reference's.** Exposing the
  ...
  different things. Same stance and wiring as FCU-FC-002's `yHtgFullOk`.
- **The chapter's asymmetries are carried, not smoothed.** This card follows the
  chapter wherever it differs from FCU-FC-002: Scope 2 against Scope 1,
...
- **The reference's runtime formula is transcribed with a caveat rather than
  ...
  dependence, and adds the scale-free enthalpy-flow expression AHU-FC-013 uses.
- **Severity 3, category, confidence and estimation method are the reference's**
  ...
  chapter README; the air-handler parent AHU-FC-013 also sits at 3.
...
- **Frontmatter `clusters`, `suppresses` and `suppressed_by` are empty.** The
  ...
  relationships to FCU-FC-002 (the mirror), FCU-FC-004 (a cooling leak, which
  makes the discharge colder and so cannot explain this fault) and FCU-FC-005 (a
