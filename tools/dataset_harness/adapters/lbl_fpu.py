...
class LoadedCase:
    ...
    def gate_mask(self, rule: str) -> list[bool]:
    ...
    def evaluable_segments(self, rule: str, inputs: list[str]) -> list[list[int]]:
        mask = self.gate_mask(rule)
        segments: list[list[int]] = []
        current: list[int] = []
        for index, (timestamp, row, gated) in enumerate(zip(self.timestamps, self.rows, mask)):
            valid = gated and all(row.get(point) is not None for point in inputs)
...
class LblFpuAdapter:
    ...
    def _parse_case(self, raw: Any) -> CaseSpec:
        ...
        expected_rules = tuple(raw.get("expected_rules", []))
        ...
        if not isinstance(startup_lead_s, int) or startup_lead_s < 0:
        ...
        for canonical, binding in raw_mappings.items():
            ...
            if canonical == "primary_airflow_reference":
                ...
                if independent is not True or "primary_airflow" in derived_from or "primary_airflow" in readiness_inputs:
                    raise DatasetError(
        ...
        if not isinstance(raw_gates, dict) or not REQUIRED_GATES.issubset(raw_gates):
            missing = sorted(REQUIRED_GATES - set(raw_gates or {}))
        ...
        for name, binding in raw_gates.items():
            ...
            applies_to = tuple(binding.get("applies_to", ["*"]))
            ...
            gates[name] = GateBinding(name, column, true_values, false_values, applies_to, description)
        ...
        return CaseSpec(
            ...
            severity=None if raw.get("severity") is None else str(raw["severity"]),
        ...
        *,
        ...
        with source_path.open(newline="") as handle:
            ...
            for row_number, source_row in enumerate(reader, start=2):
                timestamp = _parse_timestamp(source_row[case.timestamp_column], f"{source_path}:{row_number}")
                ...
                for point, binding in case.mappings.items():
                    raw_value = source_row[binding.column]
                    if raw_value.strip() == "":
                        ...
                        canonical_row[point] = convert_number(number, binding.source_unit, binding.target_unit)
                for name, gate in case.gates.items():
                    gate_values[name].append(
                        _parse_bool(
                            source_row[gate.column],
                            gate.true_values,
                            gate.false_values,
