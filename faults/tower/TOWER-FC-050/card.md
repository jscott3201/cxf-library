...
source:
  ...
  - "Sibling precedent: HP-FC-050 (commissioning-placeholder parameters, y…Ok evaluability output), HW-FC-053 (library-extension framing)"
g36: null
clusters: [CLU-10]
suppresses: []
suppressed_by: []
related: [TOWER-FC-051, TOWER-FC-052, CHW-FC-054]
...
![TOWER-FC-050 block graph](diagram.svg)
...
5. Condenser water flow above design, which raises approach as it lowers range
   (TOWER-FC-051 reads the other half of that pair, and flow is the first thing
...
- **The band is a commissioning placeholder and its fault-side corroboration is
  ...
  HP-FC-050 asks its baseline coefficients to be treated — as a value the site
...
- **The validity floor is 0 K and is not exposed as a card parameter.** It is
  ...
  sign demands it (HP-FC-050's regression slope).
- **`alarm_delay = 1800 s` departs from the hour its siblings use.** CHW-FC-053
  and HW-FC-053 require 60 minutes because their gates admit a lot of ordinary
...
- **There is no range or flow conjunct, so a flow fault reads as a tower fault.**
  ...
  TOWER-FC-051 measures the range half of that signature and the playbook orders
...
- **`clusters: [CLU-10]`.** A condenser-side syndrome (this rule, TOWER-FC-051 and
  CHW-FC-054 all describe one plant lifting harder than it should) is a
...
Where CHW-FC-054 fires and this card does not, the fouling is on the condenser
