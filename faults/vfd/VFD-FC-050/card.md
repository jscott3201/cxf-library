...
used by VAV-FC-053): `track` is the reference's `sustained for
...
- **`min_cmd_for_eval` is an adopted addition; the reference has no such
  ...
  machine behaving as designed. The 20.0% default is VFD-FC-051's `min_speed` —
...
- **The gate creates a blind spot, and not a small one.** A drive running while
  ...
  has no drive rule for; AHU-FC-052 and RTU-FC-055 ask the nearest question
...
- **Two delays in series rather than one.** The reference lists
  ...
  and a ten-minute alarm hold can have it. Precedent: VAV-FC-053, whose reference
...
- **`suppresses: [VFD-FC-051]` is an authored relationship**, not the
  reference's, which declares no suppression for either card. VFD-FC-051 reads
  ...
  so a live VFD-FC-050 makes VFD-FC-051's verdict unreliable rather than merely
  co-occurring. VFD-FC-051 carries the matching `suppressed_by`.
...
VFD-FC-051 asks a different question of the same drive — not whether it follows
