...
5. Control valve hunting downstream, modulating plant load faster than the
   boiler can follow (AHU-FC-004 is the AHU-side view of the same instability)
...
- **`min_run_time` is a tunable, not a graph parameter.** The reference lists it
  ...
  ceiling subsumes the protective intent, and RTU-FC-050 documents the identical
  gap.
- **The rolling count is built from a moving average, because the block set has
  ...
  verdict depends on where the boundary falls. AHU-FC-004 established the idiom
  and RTU-FC-050 carried it to a compressor.
...
- **The legal tick band is `57.15 s ≤ dt < 360 s`.** The floor is
  ...
  RTU-FC-050's looser `dt < 450 s` only makes the threshold exceedable at all,
...
- **The first `count_window` reads as a pace, not a count, and `alarm_delay`
  ...
  as the divisor grows). AHU-FC-004 is immune because its `alarm_delay` equals
  its `count_window`; this rule, like RTU-FC-050, is not, which is why the
...
- **`clusters: []`.** `clusters/clusters.json` defines no cluster containing a
  ...
  (Unnecessary Plant Operation) is where HW-FC-052 would belong; this fault is
...
differential, free to widen. HW-FC-051 reads the same boiler from the efficiency
side: a plant tripping both may have one problem, while HW-FC-051 alone with a
