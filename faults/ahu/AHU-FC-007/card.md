...
something removes heat (diagnoses 2, 7, 8) the unit pays twice and AHU-FC-050's
...
- **The reference card is abbreviated; G36 is the normative source.** HVAC FDD
  Reference v1.0 carries AHU-FC-007 as a §5.8.1 index row only — no equation,
...
- **The energy profile is the index row's; the emissions split is the
  ...
  `scope: 1|2` follows AHU-FC-054's convention for a fault whose fuel depends
  on which diagnosis is true; the cause-dependent runtime formula is the
  library's, its simultaneous-heating-and-cooling branch AHU-FC-050's term.
- **`SAT_AVG < SATSP − eSAT` rewritten in gap form.** Subtracting first and
  ...
  the comparison. Same rearrangement as AHU-FC-001 and AHU-FC-062, with the
...
- **`HC >= 99%` becomes a strict `>`.** CDL `Reals` has only strict
  ...
  AHU-FC-001 documents for fan speed. A host whose valve command is quantized
...
- **Instantaneous samples instead of averaged signals.** G36 computes every
  ...
  coil reads the same either way. Same note as AHU-FC-002; a SAT swinging
  across the band rather than sitting under it is AHU-FC-056's finding.
...
(AHU-FC-057) can hold a heating call the unit was never sized to meet, and a
...
whole building's heating rather than one unit's. AHU-FC-013 is this fault
mirrored into cooling; AHU-FC-005 is its companion in the same state, and
