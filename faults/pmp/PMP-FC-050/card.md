...
this rule does not read and PMP-FC-051 does: high DP with no flow is a deadhead,
...
- **`no_flow_threshold` ships an absolute placeholder, not the reference's
  ...
  loop**, as `pump_flow`'s dictionary notes require. Precedent: VAV-FC-050's
...
- **A variable-speed pump at genuine low load can sit under 5% of design flow
  ...
  differs: that case is a missing minimum-flow bypass or a deadhead, PMP-FC-051's
  territory. Sites seeing this rule fire at low load should read PMP-FC-051 first.
- **`flow_check_delay` gates the run condition, not the fault condition**, which
  ...
  Applying it to the conjunction instead — VFD-FC-050's and AHU-FC-061's shape —
...
- **The command/status mismatch is NO_EVAL here and is nobody's fault rule
  ...
  after-hours sense (AHU-FC-052, RTU-FC-055). The reference's vector table has
...
- **The flow meter is uncorroborated and the rule reports its failure as a pump
  ...
  host-side read of a dictionary point, and the reason to run PMP-FC-051 on the
  same pump.
...
- **`suppressed_by: [PMP-FC-051]` is an authored relationship**, not the
  ...
  PMP-FC-051 is the specific diagnosis where this is the general condition.
  ...
  pump DP. Precedent for authoring the edge: the VFD-FC-050/051 pair.
- **The two rules do not alarm at the same moment.** On a mid-run valve closure
  ...
  rule waits out `flow_check_delay` and lands at 360 s while PMP-FC-051 lands at
  300 s. A host implementing the suppression as "drop PMP-FC-050 if PMP-FC-051 is
  active" is right in both cases; "drop it only if PMP-FC-051 alarmed first" is
  right in one.
...
- **`g36` is null although the reference cites G36.** Its source line reads "G36
  ...
  `source` as prose. Contrast FCU-FC-005, where the reference names §5.22.6.
...
service procedure — its step 2 takes this fault and PMP-FC-051 together in the
