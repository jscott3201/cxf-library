...
reset that is missing, disabled or bottomed out too high, which is why HW-FC-057
...
diagnosis list, and the reference has no HW-FC-056 card to transcribe.
...
5. Boiler-local control ignoring the BAS — the aquastat runs its own high limit
   ...
   `hws_temp_sp`; this is the one case where HW-FC-057 stays quiet while this
   rule fires
...
(see `runtime_estimation`) by a static combustion factor, as HW-FC-052 does.

## Deviations

- **This rule is a library extension, not a transcription.** Ch.14 specifies
  HW-FC-050, 051 and 052 and publishes no card, tunables table or vectors for
...
- **`min_pump_speed_for_eval` is ADOPTED, and it is the largest departure from
  ...
  precedent is CHW-FC-053's `yLoadOk` and VFD-FC-050's `yCmdOk`.
- **The floor ships at 10%, well below the fault threshold, on purpose.** Its
  ...
  at 15% is genuinely circulating and genuinely lightly loaded. VFD-FC-050 sets
...
- **`alarm_delay` is ADOPTED at 60 minutes.** PNNL publishes no fault
  ...
  (HW-FC-052, CHW-FC-053), and it rides out the transient that would otherwise
...
- **Continuous persistence replaces PNNL's window average.** The reference
  ...
  never alarms though its window average might trip. CHW-FC-053 made the same
...
- **Pump speed alone is the load proxy; no OAT conjunct.** PNNL's neighbouring
  ...
  question. HW-FC-052 needs warm weather because it asks whether the plant should
...
- **No suppression contract with HW-FC-057, deliberately.** The two rules see one
  ...
  bottoms out at 65 °C fires this rule and not HW-FC-057, and a flat setpoint
  parked at 50 °C fires HW-FC-057 and never trips 54.4 °C here — and suppression
  is a two-card contract. They are `related` and belong in one visit.
- **`related` spans the family and is asserted here first.**
  `faults/hw/README.md`'s Relationships section documents HW-FC-050/051/052 only
  and is another writer's. One entry needs its sign stated: HW-FC-053 (low loop
...
- **`clusters: []`.** CLU-02 ("Missing Reset Strategy") is the syndrome this
  fault belongs to and already carries the air-side consequence rule AHU-FC-065,
  ...
  HW-side trigger is HW-FC-057 rather than this rule.
- **`playbooks: [hot-water-plant-faults]`.** Its Applies-To row names only
  HW-FC-050/051/052 — the index owner's line to extend — but Step 1 already
  ...
  should claim it on the hot water side is HW-FC-057.
- **Blind spots, in the order they will bite.** A constant-volume loop has no
  ...
  HW-FC-054/055's finding. The rule sees water, not setpoints, so it cannot
...
Where both fire, HW-FC-057 is the one to fix: it names the cause and this rule
