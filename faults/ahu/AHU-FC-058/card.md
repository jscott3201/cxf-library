...
source:
  - "HVAC FDD Reference v1.0 §9, AHU-FC-058"
...
suppresses: []
suppressed_by: []
related: [AHU-FC-001, AHU-FC-057, AHU-FC-065]
...
the "74% problem" with AHU-FC-057 — absent in 74% of buildings (PNNL
...
![AHU-FC-058 block graph](diagram.svg)

The setpoint chain mirrors AHU-FC-057's sampled-baseline flatness detector.
...
- **Windowed range → sampled-baseline flatness** on the setpoint chain, as
  AHU-FC-057 (`sp_flat_tolerance = min_expected_sp_range/2`; same
...
Evaluate together with AHU-FC-057: both fire → CLU-02 confirmed, one
...
separately by the after-hours rules (AHU-FC-052 family).
