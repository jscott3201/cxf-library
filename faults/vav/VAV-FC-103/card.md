...
source:
  ...
  - "Sibling precedent: VAV-FC-101/102 (this batch's recursion and reset topology), SYS-FC-055 (sub-condition flags), VAV-FC-052 (the valve seen from the command side)"
...
suppresses: []
suppressed_by: []
related: [VAV-FC-052, VAV-FC-101, VAV-FC-102, FCU-FC-002]
...
params:
  ...
  error_sigma:
    default: 0.39
    unit: "°C"
    description: "Normalization denominator, eq. 1's sigma-hat. Unlike VAV-FC-102's floor, this channel has a real healthy spread to measure and 0.39 °C is the same harness measurement — but simulation noise is cleaner than real sensors, so treat it as a starting point, not a statistic about your box."
    cxf: sigmaC.k
  slack_k:
    default: 3.0
...
energy_impact:
  ...
  runtime_estimation: "none in-rule — PROXY_ESTIMATION. This card supplies leak-hours and the rise magnitude's persistence; VAV-FC-052 carries the priced version of the same waste when the valve is driven rather than passing"
emissions:
  scope: "1|2"
...
![VAV-FC-103 block graph](diagram.svg)

The recursion, the occupancy arming and the feedback-through-`UnitDelay`
topology are VAV-FC-102's, documented there; this card adds one gate. A single
...
2. **Electric reheat element held on** — welded contactor or failed SCR, the
   ...
   safety question. FCU-FC-002 is the fan-coil sibling.
...
double-payment as VAV-FC-052, seen from the temperature side instead of the
...
- **Reheat-on clears the accumulators rather than holding them.** The source
  ...
  coil regularly commanded open is VAV-FC-052's jurisdiction, not a coil
...
- **`error_mean`/`error_sigma` are measured, not floored — the contrast with
  VAV-FC-102.** Fan heat and duct gain give this channel a real healthy
...
- **The shared house choices are inherited, not re-argued:** in-graph reset
  ...
  parameterized gains. VAV-FC-102's Deviations carry the full arguments.
- **Severity 3, `category: CRITICAL_WASTE`, `confidence: MEDIUM` and the name
  are library-authored.** CRITICAL_WASTE follows VAV-FC-052 — same waste, same
...
The trio splits by error signal: VAV-FC-101 accumulates airflow tracking
error, VAV-FC-102 zone temperature error, this card the reheat-off discharge
...
VAV-FC-052 and this card meet at the same valve from opposite sides: 052
