...
SYS-FC-054's `ambiguous` because the rule names one sensor — its two inputs are
...
- **The Ridge regression is host-side and never enters the graph.** The
  ...
  `virtual_value`, the same way HP-FC-050 and VAV-FC-050 consume host-fitted
...
- **Sibling note, not an edit:** AHU-FC-056 states that squaring inside a moving
  ...
  AHU-FC-056 should reconcile the two, though the MAD substitution is defensible
...
- **The absolute value on the bias branch is an addition.** The reference writes
  ...
  the reference gives its own paired-sensor rule SYS-FC-054. The finding does
...
- **`window` and `alarm_delay` are library-chosen, because the source publishes
  ...
  matches the 60 min `drift_duration` the reference publishes for SYS-FC-054;
  `alarm_delay = 1800 s` is SYS-FC-054's published `AlarmDelay`. A site with a
...
- **`Reals.MovingAverage`'s 64-checkpoint ring sets a minimum tick interval.**
  ...
  claims. AHU-FC-056 hit the same floor; `window` binds *both* MovingAverage
...
- **No window-fill gate in the graph**, because the block divides by elapsed
  ...
  NO_EVAL for the first `window` after a restart, the call AHU-FC-056 makes for
  its own warmup. Unlike AHU-FC-056 this rule has no ratio test, so a short
