...
AHU-FC-007's rule at zone scale; what changes is the sensor set, since an FCU has
...
- **`epsilon_sat` = 1.0 °C, adopted from G36.** The chapter's equation names
  ...
  AHU-FC-007 carries that value, and it is the library's house convention. It is
...
- **`alarm_delay` = 1800 s, adopted from G36 and not from the chapter.** The
  chapter gives FCU-FC-001 60 min and this card none, so the sibling's number is
  not automatically right. 1800 s is G36's own default and matches AHU-FC-007;
...
- **`yHtgFullOk` is exposed as a boundary output; AHU-FC-007 exposes only
  ...
  without knowing `hc_full_threshold`. Shape precedent: VFD-FC-050's `yCmdOk`.
...
- **No coil-entering temperature is available, so there is no companion test.**
  ...
  `rat` as the entering proxy (FCU-FC-004/005 use it that way) but this card's
...
Expect FCU-FC-005 on the same units in the opposite season: a heating valve worn
