...
either way, lives there and is HP-FC-050's to find.
...
power — `hp_capacity_kw` and `COP` come from the nameplate or from HP-FC-050's
...
- **A normal defrost cycle is ridden out by `alarm_delay`, and only just.**
  ...
  a false alarm, and HP-FC-051 puts `max_defrost_duration` at exactly 15 minutes.
  ...
  fault, and HP-FC-051 will be reporting it.
...
- **The 1/2 mode encoding is this library's convention**, matching the point
  ...
  the graph. Precedent: AHU-FC-063's `expected_mode`.
- **An unmapped `mode_command` leaves the rule structurally silent.** Both
  ...
  treat it as such — the same stance as AHU-FC-063, which faces the identical gap.
...
- Frontmatter `clusters` is empty: the reference lists no cluster for this fault
  and this card does not invent one. The relationship to HP-FC-050 and HP-FC-051
...
COP first, so HP-FC-050 may be reporting on the same unit, and a unit short
