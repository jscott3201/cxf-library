...
- **The comparison is an absolute difference in efficiency points, as the
  ...
  CHW-FC-050 tests a *relative* loss and HP-FC-050 multiplies its ratio through,
...
- **`efficiency_threshold` carries a fraction, not a percentage, and the failure
  ...
  HP-FC-050's `cop_ratio_threshold` has the mirror-image trap.
- **`eff_baseline_slope` and `eff_baseline_intercept` ship as documented
  ...
  alarms; fitted low, nothing ever does. Precedent: HP-FC-050's COP line,
...
- **The slope may be negative — the documented exception to the library's
  ...
  without rewiring. HP-FC-050 carries the identical exception.
- **`alarm_delay` is adopted, not transcribed, because the source line
  ...
  authorities — CHW-FC-050's `AlarmDelay = 60 min` on the same
  fitted-baseline shape, and HP-FC-050 — and is the shortest delay that reliably
...
- **The fitted line is extrapolated without limit.** Nothing in the graph knows
  ...
  HP-FC-050 documents the same open end.
