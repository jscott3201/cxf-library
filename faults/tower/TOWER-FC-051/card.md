...
source:
  ...
  - "Sibling precedent: CHW-FC-053 and HW-FC-053 (delta-T graph shape, evaluability output), HP-FC-050 (commissioning-placeholder contract)"
g36: null
clusters: [CLU-10]
suppresses: []
suppressed_by: []
related: [TOWER-FC-050, TOWER-FC-052, CHW-FC-053]
...
params:
  ...
  alarm_delay:
    default: 3600.0
    unit: s
    description: "Continuous range collapse at fan load required before the alarm asserts (60 min). ADOPTED from CHW-FC-053 — no tower source specifies a persistence, and condenser-loop thermal mass plus chiller staging make anything shorter noise."
...
energy_impact:
  ...
  runtime_estimation: "excess_cond_pump_kw ≈ cond_pump_kw × (design_range − actual_range) / design_range — CHW-FC-053's estimator read on the condenser loop, and valid only for the flow-driven case. Neither cond_pump_kw nor design_range is a point of this rule or of the tower dictionary, so the host supplies both; actual_range is the difference the graph already computes"
emissions:
  scope: "2"
...
![TOWER-FC-051 block graph](diagram.svg)
...
CHW-FC-053's estimator on the condenser loop. Where it is a bypass or a
...
- **The band is a commissioning-set placeholder, and its only quantitative
  ...
  **CTI/ASHRAE fault-side corroboration is pending.** Until a site commissions the
  ...
  HP-FC-050's contract, and the reason `confidence: LOW`.
- **One absolute band, not a design × fraction pair.** CHW-FC-053 and HW-FC-053
...
- **Range is not a tower-capability measurement, and the flow confound cannot be
  ...
  *capability* degradation is TOWER-FC-050's approach test, not this one.
- **The fan-speed floor is entirely adopted.** No tower source gates a range or
  ...
  HW-FC-053 records for PNNL-27338 §4.6. 30% is chosen against typical tower VFD
...
- **There is no chiller-on conjunct, and that is a real blind spot.** A condenser
  ...
  through the off-cycles of a normally staging plant; HW-FC-053 rejected the same
...
- **`yFanOk` is an evaluability flag, not a sub-condition flag.** False means
  ...
  healthy tower. Same stance as CHW-FC-053's `yLoadOk` and HP-FC-050's `yPowerOk`.
- **`alarm_delay = 3600 s` is adopted from CHW-FC-053, not sourced.** No tower
...
- **`clusters: [CLU-10]`.** `clusters/clusters.json` has no condenser-side cluster; CLU-06
  ...
  TOWER-FC-050 and CHW-FC-054 all describing one condenser loop giving away
  energy) is a reasonable future cluster and the cluster owner's edit.
- **`suppresses` and `suppressed_by` are both empty.** TOWER-FC-050 is the closest
...
and the distribution basin. Where TOWER-FC-050 fires as well, the approach finding
