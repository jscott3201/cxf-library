...
the heating mirror of SYS-FC-050, and the reference writes it that way — "same
...
2. Leaking heating coil valve(s) — a valve commanded shut that does not seat
   ...
   be cooled back down; AHU-FC-015 sees it from the air side
...
- **`no_demand_flow_threshold` ships as a placeholder, and the mirror makes it
  worse here.** The reference gives SYS-FC-050 `10% of design` and gives this
  ...
  silent. **Fit it per loop before deployment.** Precedent: VAV-FC-050's
...
- **No schedule, occupancy, or OAT gate.** The reference puts none in this
  equation, and the weather-based version of the fault is HW-FC-052 (plant
...
and 3. Pull HW-FC-052 alongside: it asks whether the plant is running above its
...
too — CLU-07's trigger is SYS-FC-050, and the sequence gap is usually written
once and copied.
