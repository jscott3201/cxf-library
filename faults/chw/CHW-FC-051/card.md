...
than 30% of buildings — the plant-side half of the story AHU-FC-057 and
AHU-FC-058 tell about air handlers, and the fix is desk work.
...
This is AHU-FC-057's detector with the plant's points bound to it. Two symmetric
...
- **Windowed range → deviation from a weekly sampled baseline.** The reference
  ...
  flat. AHU-FC-057 is the precedent and the mechanism is copied unchanged.
- **`Reals.MovingAverage` rejected, and the tick band that follows.** The engine
  ...
  BAS ticks that slowly; AHU-FC-057 found this and this card inherits it. The
...
- **`min_load_range` is adopted, not transcribed.** The chapter names it in the
  ...
  follows AHU-FC-057's `min_oat_range_for_eval` in shape and intent: a low bar a
...
- **`AlarmDelay` = 24 h implemented as `TrueDelay` on the fault conjunction**;
  ...
  AHU-FC-050), so a rule loaded into an already-faulted plant still waits the
...
- **Blind spots.** The rule reads the setpoint, never the water: a plant whose
  setpoint moves while the chillers ignore it is CHW-FC-050's problem.
...
an AHU-scoped cluster triggered by AHU-FC-057, and membership is
...
CHW-FC-051 and CHW-FC-052 has one root cause, no reset strategy ever
