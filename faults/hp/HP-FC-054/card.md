...
source:
  ...
  - "Library-authored: the HVAC FDD Reference v1.0 ch.11 specifies no refrigerant-side heat pump rule (HP-FC-050..052 only); name, severity 3 and method: rule are argued on this card"
  - "Sibling precedent: RTU-FC-051 (a fixed baseline shipped as a named simplification of a fitted model), HP-FC-050 (placeholder parameter with a silent-failure warning), HP-FC-051 and HP-FC-052 (defrost and mode gating kept host-side)"
g36: null
clusters: []
suppresses: []
suppressed_by: []
related: [HP-FC-050, HP-FC-053]
...
an efficiency rule such as HP-FC-050 will usually never see it.
...
![HP-FC-054 block graph](diagram.svg)
...
commercial packaged heat pump — HP-FC-050's range scaled down by this fault's
...
- **A fixed nominal band replaces the source's regressed reference model — the
  ...
  RTU-FC-051 names the same class of substitution for its fixed split baselines.
...
- **No superheat conjunct, by evidence rather than convenience.** The source's
  ...
  companion undercharge rule (HP-FC-053) is where superheat carries
...
- **The rule stands alone; the source runs its chart only after an efficiency
  ...
  coverage this card adds over HP-FC-050.
...
- **Compressor, mode and defrost gating stay host preconditions**, per the
  ...
  is noise to this rule (HP-FC-051 and HP-FC-052 precedent).
...
Read alongside HP-FC-050, which will usually be silent here by design, and
HP-FC-053, the undercharge branch — the two firing together is a contradiction
