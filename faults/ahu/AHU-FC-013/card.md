...
This is G36 §5.16.14 FC#13, the exact mirror of AHU-FC-007 on the heating side.
...
rule and AHU-FC-012 together is pointing at those two rather than at the coil.
...
immediate and AHU-FC-050's term sizes it. If the coil, the chilled water, or
...
behind the fault, but that is heat this rule cannot see; AHU-FC-012 measures it
...
- **The reference card is abbreviated; G36 is the normative text.** The HVAC
  FDD Reference carries AHU-FC-013 only as a §5.8.1 index row — no equation,
...
- **Setpoint comparison rewritten as gap comparison.** Subtracting first and
  ...
  of a two-signal comparison. Same rearrangement as AHU-FC-001 and AHU-FC-062.
...
- **`CC >= 99%` becomes a strict `> 99.0`.** CDL `Reals` offers only strict
  ...
  retune hint as AHU-FC-001 and AHU-FC-007: a host binding a coarsely quantized
...
- **Instantaneous samples instead of 5-minute rolling averages.** G36 computes
  ...
  hunting SAT loop, which is AHU-FC-056's fault to report. A steady miss
...
- **The energy profile is the index row's; the runtime formula and scope are
  ...
  gives AHU-FC-007, this rule's heating-side mirror. The branched formula is
  ...
  Scope 1 half of the shared diagnoses to AHU-FC-012.
...
reset has been disabled or never commissioned — the condition AHU-FC-057
...
together with AHU-FC-012: every diagnosis here appears there, and what differs
