...
stance is AHU-FC-062's and RTU-FC-052's: delivery quality stays host-owned,
...
Confidence is MEDIUM rather than LOW, a deliberate departure from AHU-FC-062 and
RTU-FC-052: those compare several sensors and cannot say which is wrong, while
...
AHU-FC-062 records it that way: it depends on which subsystem the bad number
...
- **`sample_period` and `history_warmup` are two card parameters carrying one
  ...
  form AHU-FC-057 uses is unavailable, because the two block parameters take
  ...
  coupled-parameter hazard as HP-FC-050's slope and intercept.
...
- **`adjudicates` cards must not be suppressible, so `suppressed_by` is empty
  ...
  point. AHU-FC-062's fourteen-entry `suppresses` list is the counter-example:
