...
4. Defective run capacitor: the compressor stalls on start and drops out on
   thermal overload
5. Iced or fouled evaporator coil starving the suction side (RTU-FC-051 sees the
...
Scope 2, QUALITATIVE_EMISSIONS, MEDIUM confidence. AHU-FC-001's convention gives
...
- **This rule needs a faster tick than the rest of the library, and the reason
  ...
  therefore 60.0 (a 60 s tick) rather than AHU-FC-004's 12.0. Combined with the
...
- **`count_scale` is coupled to the host's tick interval and the failure is
  ...
  alarms on four starts an hour. AHU-FC-004's deployment constraint verbatim.
- **Minimum tick interval, from the moving average's ring.** Each
  ...
  checkpoints, so `dt ≥ 3600/63` ≈ **57.2 s**. (AHU-FC-004 quotes `delta/64` =
...
- **Rolling count built from a moving average, because the block set has no
  ...
  count whose verdict depends on where the hour boundary fell. AHU-FC-004's
  trade, taken again.
...
- **Startup artifact (b): the first hour reads as a rate, and here it can reach
  ...
  extrapolated. Unlike AHU-FC-004, `alarm_delay` (15 min) is shorter than the
...
- **The reference tags this fault for both RTU and HP.** This card is the
  RTU-family instance (AHU-FC-059 precedent); the heat-pump sibling would
...
start, widen the differential. Firing together with RTU-FC-051 points at the
