...
source:
  ...
  - "Sibling precedent: CHW-FC-053 (graph shape, two-parameter trip line, yLoadOk evaluability output), HP-FC-050 (commissioning-placeholder contract)"
  - "Library extension: the HVAC FDD Reference v1.0 ch.13 specifies CHW-FC-050 through 053 only"
g36: null
clusters: [CLU-06, CLU-10]
suppresses: []
suppressed_by: []
related: [CHW-FC-050, CHW-FC-053, TOWER-FC-050, HP-FC-053]
...
params:
  ...
  min_load_for_eval:
    default: 40.0
    unit: "%"
    description: "Chiller load below which approach is not evaluated. Approach scales with the heat flux across the tubes, so a lightly loaded machine makes a small approach whether its condenser is clean or fouled. ADOPTED from CHW-FC-053's floor for the same chiller_load point; the sources supply no load gate of their own."
    cxf: loadOk.t
  alarm_delay:
    default: 3600.0
    unit: s
    description: "Continuous high approach at load required before the alarm asserts (60 min). ADOPTED from CHW-FC-053; fouling is a condition of the tubes, not an event, and an hour rides out a capacity step or a condenser water reset."
...
![CHW-FC-054 block graph](diagram.svg)
...
a tower that cannot make its setpoint is TOWER-FC-050's finding, not this one.
...
3. Condenser water flow below design — a throttled or failing pump, a fouled
   ...
   different repair, and CHW-FC-053's low delta-T sibling on the condenser side
...
Scope 2, PROXY_EMISSIONS, MEDIUM confidence; the same order as CHW-FC-050's
...
- **The band is a commissioning placeholder, not a threshold.** Three deep-read
  ...
  in the same sense HP-FC-050's shipped regression coefficients do.
...
- **`min_load_for_eval` is entirely adopted; no source gates this test.** 40% is
  CHW-FC-053's floor on the same `chiller_load` point, so the two chiller rules
...
- **`yLoadOk` is an evaluability output, not an echo of an input** — a boundary
  ...
  CHW-FC-053's `yLoadOk` and HP-FC-050's `yPowerOk`.
...
- **`clusters: [CLU-06, CLU-10]` with CLU-06 the obvious candidate.** `clusters/clusters.json`
  groups the chilled water plant syndrome behind CHW-FC-050 as trigger, and a
...
- **`playbooks` cites two and only one names this card back.**
  `cooling-tower-performance`'s Applies-To row already carries CHW-FC-054 and its
  Step 1 describes this test correctly; `chiller-efficiency`'s row does not yet,
  and that edit belongs to the index owner — the sequencing CHW-FC-053 and
  HW-FC-053 both recorded.
...
Read `yLoadOk` before `yFault`, and read this rule next to CHW-FC-050. A fouled
...
route HP-FC-053 takes on packaged equipment. Trend approach against load for a
