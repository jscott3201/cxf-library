...
shows up in ERV-FC-050's effectiveness test afterwards, as a consequence rather
than a coincidence.
...
- **`erv_enabled` is a conjunct in the graph, not a host-side operating-state
  ...
  what the reference states. ERV-FC-050 makes the same choice with the same
...
- **No evaluability output.** SCHEMA.md asks for one when the reference's
  ...
  directly. Contrast ERV-FC-050's `yTempDeltaOk`, which is computed from three
...
- Frontmatter `clusters` is empty: the reference defines no cluster containing an
  ...
  ERV-FC-050 is carried by `related` and the shared playbook.
...
document is written around a fouled or stalled recovery device (ERV-FC-050) and
