...
- **`reheat_active_threshold` is adopted, not transcribed.** It appears in the
  ...
  value the same chapter gives VAV-FC-055's identically-named parameter, which
...
- **`zone_airflow` is dropped from the points list.** The reference's points
  ...
  ignores. Precedent: AHU-FC-063 drops `oat` for the same reason.
...
- **Frontmatter `clusters` is empty.** CLU-05 covers this fault's neighbours
  (VAV-FC-052, VAV-FC-055, SYS-FC-056) but chapter 7 does not list VAV-FC-050
...
air is too cold (AHU-FC-053). PNNL-27338's AIRCx check is the sharper version —
...
time. Clearing this fault should also quiet VAV-FC-055 on the same box, since an
oversized minimum is its diagnosis 1; it will not quiet VAV-FC-052, which sees a
