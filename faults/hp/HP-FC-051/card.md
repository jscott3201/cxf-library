...
4. Refrigerant charge issue — low charge lowers coil temperature and brings on
   ...
   capacity (HP-FC-050 is the rule that sees it as capacity)
...
- **`comp_status` is dropped from the points list.** The reference's Required
  ...
  here rather than binding a point the graph ignores. Precedent: VAV-FC-050
  drops `zone_airflow`, AHU-FC-063 drops `oat`.
...
- **The rolling count is built from a moving average, because the block set has
  ...
  tumbling window whose verdict depends on where the boundary fell. AHU-FC-004
...
- **The first hour reads as a rate, not a count, and `alarm_delay` does not
  ...
  the pace, extrapolated. AHU-FC-004's `alarm_delay` equals its `count_window`,
...
heating hours in defrost. Check HP-FC-050 on the same unit before ordering a
