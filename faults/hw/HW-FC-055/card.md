...
This is CHW-FC-051's detector with the heating loop's points bound to it. Two
...
`pump_waste_kw = hw_pump_kw × [1 − (1 − DP_reduction/100)³]`, CHW-FC-052's
...
unlike HW-FC-057's fuel-side twin, which a site owns however clean its
electricity gets. Typical range 300–3,000 kg CO₂e/yr by analogy with
CHW-FC-052's chilled water case; no HW-specific figure is published.
...
- **Daily MAX−MIN → rolling sampled baseline plus dwell.** PNNL's reset checks
  ...
  AHU-FC-057/058 and again at CHW-FC-051/052. Detection is equivalent for a
...
- **Pump-speed range is the activity conjunct, and it is not CHW-FC-052's valve
  ...
  reset here. HW-FC-054 (high loop DP) separates those two on the same loop.
- **The 20-point minimum speed range is adopted, and argued rather than cited.**
  ...
  `pump_variation_tolerance = 10 %` (half-range) follows CHW-FC-051's adopted
...
- **NO_EVAL is surfaced as `yPumpSpeedVaried`, where CHW-FC-052 ships no
  ...
  CHW-FC-051's `yLoadVaried` semantics exactly, and it earns its place as an
...
- **`Reals.MovingAverage` rejected, and the tick band that follows.** The engine
  ...
  BAS ticks that slowly; AHU-FC-057 found this and every reset rule since has
...
- **`alarm_delay` = 24 h implemented as `TrueDelay` on the fault conjunction**;
  ...
  AHU-FC-050), so a rule loaded onto an already-faulted loop still waits the
...
`clusters/clusters.json`'s to declare — the same call CHW-FC-051 and CHW-FC-052
made one system upstream. A plant failing both HW-FC-055 and HW-FC-057 has one
