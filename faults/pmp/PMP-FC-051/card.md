...
DP by itself is a loop at high head, low flow by itself is PMP-FC-050's
...
- **`alarm_delay` has no published value, and no published existence.** The
  ...
  truncation artifact VFD-FC-051's line carries. Both the persistence and its
  value are therefore ADOPTED, at PMP-FC-050's published 300 s: same chapter,
  same family, same physical event from a second angle. VFD-FC-051's 900 s was
...
- **No evaluability output, deliberately.** `pump_status` is a boundary input, so
  ...
  could not compute for itself. Contrast PMP-FC-050's `yRunOk`, a conjunction
  held for a delay, and VFD-FC-050's `yCmdOk`.
- **`suppresses: [PMP-FC-050]` is an authored relationship**, not the
  ...
  event and this one is the specific diagnosis where PMP-FC-050 is the general
  ...
  the edge: the VFD-FC-050/051 pair.
...
- **A constant-speed pump deadheads differently from a variable-speed one.** On
  ...
  drive-controlled loop. VFD-FC-051 is `related` for this reason.
...
- **Persistence stands in for averaging.** The rule consumes instantaneous points
  ...
  same blind spot PMP-FC-050 documents.
- **The energy formula's inputs are not this rule's inputs.** `waste_kw =
  ...
  unchanged otherwise, including its identity with PMP-FC-050's.
...
- The reference publishes a vector table for PMP-FC-050 and none for this card,
...
finding and PMP-FC-050 is its shadow — the two diagnosis lists point in opposite
