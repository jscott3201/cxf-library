...
MEDIUM rather than AHU-FC-055's HIGH on the same quotient, because a low-side
...
follows the season, as for AHU-FC-055: excess outdoor air burns Scope 1 fuel at
...
- **The reference card is abbreviated; G36 is the normative text.** The HVAC
  FDD Reference carries AHU-FC-006 only as a §5.8.1 index row — a name and an
...
- **`%OAmin` is a static parameter, not the active setpoint.** G36 defines it
  ...
  a VAV system recomputes continuously; binding it as a constant (as AHU-FC-055
...
- **Operating states and ModeDelay are host-side preconditions.** G36 scopes
  ...
  library's stance (precedent AHU-FC-063). The stakes are higher here than on
...
- **The energy profile is the index row's; the emissions block and runtime
  ...
  `PROXY_EMISSIONS` follow AHU-FC-055's reading of the same physics, and the
  formula is mirrored from AHU-FC-055 with the sign check added.
...
AHU-FC-055 is a one-sided energy test with a 0.10 tolerance in any
non-economizing occupied hour; AHU-FC-006 is G36's symmetric test in the two
...
contradiction that AHU-FC-062 tests directly with a shorter delay. Expect the
pair to fire together and read AHU-FC-062 first; a genuine under-ventilation
